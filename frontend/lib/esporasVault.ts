...
/** Una plantilla: es una nota normal, así que su título es el nombre del archivo. */
...
/**
 * Id de la carpeta configurada, o `null` si todavía no existe.
 *
 * > [!important] Acá la web NO puede copiar al escritorio
 * > En desktop el id de una carpeta **es su ruta**, así que alcanza con comparar
 * > contra `carpetaEsporas()`. Acá el id es un **UUID** que no guarda relación con
 * > el nombre, de modo que la ruta configurada hay que resolverla recorriendo el
 * > árbol segmento a segmento desde la raíz. Es el único punto de las Esporas
 * > donde las dos capas de datos no son intercambiables.
 */
export function idCarpetaEsporas(
  carpetas = useVaultStore.getState().carpetas,
): string | null {
  let actual: string | null = null;
  for (const segmento of carpetaEsporas().split("/")) {
    const hija: TreeCarpeta | undefined = carpetas.find(
      (c) => c.padreId === actual && c.nombre === segmento,
    );
    if (!hija) return null;
    actual = hija.id;
  }
...
export function listarEsporas(notas = useVaultStore.getState().notas): Espora[] {
  const carpetaId = idCarpetaEsporas();
  if (carpetaId === null) return [];
  return notas
    .filter((n) => n.carpetaId === carpetaId && n.tipo === "markdown")
...
export async function asegurarCarpetaEsporas(): Promise<string> {
  ...
  for (const segmento of ruta.split("/")) {
    recorrido = recorrido === "" ? segmento : `${recorrido}/${segmento}`;
    const hija = () =>
      useVaultStore
        .getState()
        .carpetas.find((c) => c.padreId === padreId && c.nombre === segmento);
    if (!hija()) {
      await useVaultStore.getState().createCarpeta(segmento, padreId);
      if (!hija()) {
        throw new Error(
          `No se pudo crear la carpeta «${recorrido}»: ya hay una nota o carpeta con ese nombre.`,
        );
      }
    }
    padreId = hija()!.id;
