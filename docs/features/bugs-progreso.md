...
| DEF-048 | Falta margen inferior en toda la app: el contenido queda pegado al borde | ambas (frontend) | ✅ ambas (2026-08-13) — token `--mic-gap-inferior`; **confirmado en la app** el 2026-09-05 y reflejado a web |
...
| DEF-053 | Las opciones del grafo se salen de la pantalla si la pestaña es pequeña | ambas (frontend) | ✅ ambas (2026-08-13) — portal y posición acotada; **confirmado en la app** el 2026-09-05 y reflejado a web. Su patrón —el portal al `body`— se reusó después en `DEF-077`, `FUN-S-16` y `FUN-S-12` |
| DEF-054 | El grafo no se actualiza si el archivo lo crea algo de fuera de Mycelium | desktop | ✅ desktop (2026-08-13) — el watcher marca el grafo desactualizado; **confirmado en la app** el 2026-09-05. **Sin reflejo**: en web no hay watcher, porque no hay carpeta que vigilar |
...
| DEF-088 | Plegar un título hace desaparecer su flecha; al verla, apunta como desplegada. Tooltip en inglés | ambas (frontend) | ✅ desktop en `experimento/ui-impeccable` (2026-09-19), **confirmado en la app** por el usuario; sin reflejar a web — el CSS leía el `title` de CodeMirror en el elemento equivocado y con otra mayúscula; ahora la marca propia lleva `data-plegado` y los textos de CodeMirror están en español. Ver [[CodeMirror y la vista en vivo]] |
| DEF-089 | En vivo, un `[[enlace]]` dentro de código en línea se decora como enlace | ambas (frontend) | ⏳ pendiente (registrado 2026-09-19) |
| DEF-090 | La búsqueda de la barra superior no busca: se escribe y no pasa nada | ambas (frontend) | ✅ corregido en `experimento/ui-impeccable` (2026-09-19, rediseño del cascarón). ✅ **confirmado en la app** (2026-09-20): el campo abre la paleta, «arquitect» devuelve la nota, «>» los 11 comandos, vacío las recientes, y Escape cierra — se decidió que el campo sea el disparador de una paleta: notas por nombre (Ctrl+O) y, con «>», comandos (Ctrl+P). Causa: el `<input>` de `AppTopbar` nunca tuvo manejador; era un marcador de la maqueta original. Ver [[Rediseñar la UI con impeccable]] |
| DEF-091 | En los temas claros, el código en línea no se lee (1.04:1 en lectura) | ambas (frontend) | ✅ corregido en `experimento/ui-impeccable` (2026-09-19). ✅ **confirmado en la app** (2026-09-20): 11.49:1 y 12.27:1 en claro (era 1.04:1), medido en los dos temas y en dos atmósferas — `--mic-bg-code` seguía oscuro en claro y se escribía con la tinta oscura; además los bloques del modo vivo pintaban la paleta de sintaxis clara sobre ese fondo oscuro. En claro el código pasa a un fragmento claro; la paleta clara se recalibró a ≥4.5:1 y la lectura dejó el tema atom-one-dark por las mismas `--mic-syntax-*`. Medido: mínimo 4.63:1 en lectura y vivo, cuatro combos |
| DEF-092 | La vista de lectura no limita el ancho: renglones de 170-180 caracteres | ambas (frontend) | ✅ corregido en `experimento/ui-impeccable` (2026-09-19). ✅ **confirmado en la app** (2026-09-20): el renglón mide 662px con `--mic-ancho-lectura: 42rem`, y el canalón de plegado viaja con él (padding 269.7 / 284) — el selector `.mic-layout-read .mic-preview > div` buscaba una clase dentro de la otra, pero la lectura, la vista lateral y la vista previa enlazada ponen las dos en el MISMO elemento: nunca coincidió. Pasa a `.mic-preview.mic-layout-read > div`. Medido: columna de 590px centrada a 1536px de ventana, renglones llenos de 72-81 caracteres |
| DEF-093 | Tabla en vivo: celda sombreada vacía a la izquierda del encabezado | ambas (frontend) | ✅ corregido en `experimento/ui-impeccable` (2026-09-19). ✅ **confirmado en la app** (2026-09-20): el `<th>` del carril devuelve `rgba(0, 0, 0, 0)` — es el carril de los tiradores de fila (`.mic-tab-lat`): en el encabezado es un `<th>` y tomaba el fondo de `.mic-preview th`. Pasa a fondo transparente |
| DEF-094 | El título de la ventana muestra la ruta completa del vault, no su nombre | desktop | ✅ corregido en `experimento/ui-impeccable` (2026-09-19). ✅ **confirmado en la app** (2026-09-20): la ventana se llama «Mycelium - Mycelium», el nombre de la carpeta y no `C:\Trabajo\GSmart\Mycelium` — `nombreDeVault` partía con `/[/]/`, que es solo la barra común: en una expresión regular la contrabarra hay que escaparla. Con una ruta de Windows no partía nada y devolvía la ruta entera. Se notó al poner el nombre del vault en la barra superior ([[marco-de-ventana]]) |
...
| DEF-099 | Las consolas no pertenecen al vault: al abrir otro siguen las del anterior, apuntando a su carpeta | desktop | ✅ desktop (2026-09-22), **confirmado en la app**. `terminalStore` persistía en **una sola** clave (`mic-terminales`), sin vault ni ventana; ahora es una por vault (`mic-consolas:<ruta>`), como `DEF-044` hizo con las pestañas. Al cambiar de vault se **matan los PTY** —su cwd es de la carpeta que se deja— pero la lista se conserva en la clave de ese vault, así que al volver están sus consolas, como tras reabrir la app (CA6). Las **preferencias** salieron a su propia clave: son del usuario, no del vault. **Sin reflejo**: la terminal es solo-desktop |
| DEF-101 | Un clic en un enlace web se lleva la ventana de Mycelium a esa página (en lectura); en edición en vivo no hace nada | ambas (difiere) | ✅ desktop (2026-09-23), **confirmado en la app**. El manejador de clics solo entendía `#wikilink:` y `#tag:`, así que un `http(s)://` navegaba la webview entera; en la edición en vivo **no había ningún `<a>`** —el live preview oculta el `(url)`— y esa era la mitad callada. Se corrige con `FUN-S-20`: un helper único para las cinco vistas, más `on_navigation` en Rust como red de seguridad. Tardó dos pasadas: la primera dejó el comando habilitado **con el alcance vacío** (`opener:allow-open-url` sin `opener:allow-default-urls`), así que el clic no hacía nada. ✅ **reflejado a web el 2026-09-23** (`a83267d`), **adaptado**: en web no hay plugin `opener`, así que `abrirEnNavegador` hace `window.open(destino, "_blank", "noopener,noreferrer")` y la red de seguridad de Rust no va. Allá el defecto **no es grave** —navegar es lo normal en una pestaña del navegador y el botón de atrás vuelve—, pero abrir en pestaña nueva sigue siendo lo correcto para una aplicación de una sola página. Lo que se conserva igual es la lista de esquemas y que el manejador sea **uno solo** para las cinco vistas |
| DEF-100 | Varias ventanas de Mycelium comparten la lista de consolas | desktop | ✅ desktop (2026-09-22), **confirmado en la app**, con el mismo cambio que `DEF-099`: el `localStorage` es del **origen**, así que lo ve toda ventana, y lo único que las separa es la clave. Con una por vault no pueden colisionar, porque un vault solo se abre en una ventana (`FUN-L-16`). En Rust las sesiones ya estaban separadas (cada `Sesion` recuerda su ventana y muere con ella): lo compartido era la **lista**, no el proceso |
...
- **DEF-065 · DEF-075 — el plegado estaba atado a la identidad del DOM, no a la del documento.**
  ...
  elementos `<h1>`…`<h6>` en un `WeakMap` por contenedor, y podaba los que ya no colgaban de
  él. Cualquier cosa que reescribiera el HTML del preview crea nodos **nuevos**, así que los
  guardados se descartaban: al cambiar de pestaña (`DEF-065`) y también solo, al poco rato,
  cuando un autoguardado re-renderiza el preview (`DEF-075`). El mismo `delete`.

  Ahora un título se identifica por **nivel + texto**, con un contador para los repetidos. Se
  descartó el índice posicional: es más simple pero se rompe al editar otra parte del
  documento, porque agregar un título arriba corre todos los de abajo. **El precio** del texto
  es que renombrar un título plegado despliega esa sección; es el caso raro.

  En **edición** el mecanismo es otro: el plegado vive en el estado de CodeMirror y la vista se
  destruye al cambiar de pestaña. Se guardan los rangos con `foldedRanges` y se reponen con
  `foldEffect`, solo si el documento es el mismo (`restaurable`) — la misma condición que ya
  gobernaba el cursor y el scroll, porque un rango viejo sobre otro texto plegaría cualquier
  cosa.

  Los dos conjuntos van al `instanceCache`, y ahí la vida pedida por el `DEF-065` sale gratis:
  sobrevive a cambiar de pestaña y muere al cerrarla, porque al reabrir la nota la pestaña es
