...
| DEF-099 | Las consolas no pertenecen al vault: al abrir otro siguen las del anterior, apuntando a su carpeta | desktop | ⬜ pendiente (registrado 2026-09-22) — mismo origen que `DEF-100`: `terminalStore` persiste en **una sola** clave de `localStorage` (`mic-terminales`), sin vault ni ventana. `DEF-044` ya hizo por vault el almacén de pestañas, y ahí las terminales quedaron **excluidas a propósito** del descarte («no son notas del vault»); esa decisión es la que el usuario está pidiendo revisar |
| DEF-100 | Varias ventanas de Mycelium comparten la lista de consolas | desktop | ⬜ pendiente (registrado 2026-09-22) — la otra cara de `DEF-099`: el `localStorage` es del origen, así que lo ve toda ventana. En Rust las sesiones **sí** están separadas (cada `Sesion` recuerda su ventana y mueren con ella, `FUN-L-16`), de modo que lo compartido es la **lista**, no el proceso |
