...
| DEF-101 | Un clic en un enlace web se lleva la ventana de Mycelium a esa página (en lectura); en edición en vivo no hace nada | ambas (difiere) | 🛠️ desktop (2026-09-23), **implementado sin confirmar** — nadie interceptaba el clic: el `<a href>` llegaba al navegador embebido, que navegaba la webview. La decisión pasa a `lib/enlacesExternos.ts` y la usan los **cinco** sitios que atienden clics (lectura, vista en vivo, tablas, propiedades, canvas); arreglar uno solo dejaba enlaces que abren bien en una vista y se llevan la app en otra. La vista en vivo no tenía `<a>` —el live preview oculta el `(url)`—, así que el nodo `Link` se marca con su `data-href`. Solo se abren `http`, `https` y `mailto`. Red de seguridad en Rust (`navegacion.rs`, `on_navigation`) para los caminos olvidados. Ver [[enlaces-externos]]. **Falta confirmarlo en la app**: que el navegador abra de verdad necesita Tauri corriendo. **En web el síntoma es otro** —navegar es lo normal en una pestaña, y el botón de atrás vuelve—, así que allá alcanza con `target="_blank"` |
