...
| DEF-101 | Un clic en un enlace web se lleva la ventana de Mycelium a esa página (en lectura); en edición en vivo no hace nada | ambas (difiere) | ⬜ pendiente (registrado 2026-09-23) — el manejador de clics de las notas solo entiende `#wikilink:` y `#tag:`, así que un `http(s)://` navega la webview entera. Se corrige junto con `FUN-S-20`, que es la mitad que falta: abrir el navegador del sistema. **En web el síntoma es otro** —navegar es lo normal en una pestaña, y el botón de atrás vuelve—, así que allá alcanza con `target="_blank"` |
