...
| [[bases-tabla]] (`FUN-L-03`) | Sale en la [[Version 1.6.0]]. **Implementada en las dos ramas** el 2026-08-08, **sin confirmar**: el archivo `.base` (formato de Obsidian) agrega notas por sus propiedades y las muestra en una tabla de solo lectura, con filtros. Se implementa un subconjunto cerrado del lenguaje de Bases; lo que no se entiende **no se ignora**, se declara. El **constructor de filtros** se rehizo el 2026-09-05 (`FUN-M-27` + `FUN-S-16` + `DEF-080`, confirmado en la app y reflejado a web): de lista plana a árbol, con negación, grupos anidados y buscador de campos. Queda pendiente la otra mitad del bloque **O**, la cabecera de la tabla (`FUN-S-14`, `FUN-S-15`, `FUN-M-25`). Sin release todavía |
...
> [!success] El hueco de publicación se cerró el 2026-08-17
> Durante dos semanas el pendiente número uno fue publicar: la 1.6.0 y la 1.6.1 estaban
> numeradas pero nunca salieron, así que quien se actualizaba seguía recibiendo la 1.5.0,
