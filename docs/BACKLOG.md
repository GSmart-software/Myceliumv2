...
| `FUN-L-11` 🛠️ | `FILES-OTROS-TIPOS` | **Abrir** los archivos que no son notas —texto, código, PDF e imágenes— en un visor propio, como una pestaña más. La mitad de *aparecer en el explorador* ya la resolvió `FUN-S-03`. **De solo lectura** y **solo-desktop**: en web esos archivos no existen en ninguna parte. **Implementado en desktop** el 2026-08-22 (sin confirmar): pestaña `archivo:<ruta>` por centinela, texto por un comando propio que corta por tamaño y detecta lo que no decodifica, PDF/imagen por el protocolo `asset:` acotado a la carpeta del vault. Spec en [[otros-tipos-de-archivo]] | desktop | — |
