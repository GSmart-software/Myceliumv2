...
| `FUN-M-21` 🛠️ | `GRAPH-NOMBRES-SEGUN-FOCO` | Tres modos para los nombres del grafo: **todos**, **solo el nodo apuntado y sus vecinos**, o **solo el apuntado**. Con mucha densidad de nodos, todos los nombres a la vez entorpecen la vista. La elección **persiste por vault**. **Implementado en desktop y confirmado en la app** el 2026-09-05: el control va en el menú del grafo —es una opción de esa vista— y «vecino» es cualquiera con el que el foco conecte, en las dos direcciones. Sobre [[preferencias-por-vault]], que **no se refleja tal cual a web** | ambas | — |
...
> | Grupo | Qué comparten |
> |---|---|
> | **Preferencias por vault** — `FUN-M-28` + `FUN-M-21` ✅ | Las dos decían «persiste por vault» y **eso no existía**: las preferencias eran por usuario. Se creó el almacén y las dos van encima ([[preferencias-por-vault]], 2026-09-05). Le queda servida la casa al ancho de columnas de `FUN-M-25`, cuya única razón de ser `M` era no saber dónde guardarlo |
...
> [!success] La mitad del constructor está hecha (2026-09-05)
> `FUN-M-27` + `FUN-S-16` + `DEF-080` salieron juntas y están confirmadas en la app, en un
> componente nuevo (`components/bases/FiltrosBuilder.tsx`). Haberlas hecho en una sola
> pasada fue lo correcto: el árbol de `FUN-M-27` reescribió el constructor entero, así que
> el buscador y la marca de incompleta habrían tenido que rehacerse igual. Queda **la otra
> mitad**, la cabecera de la tabla: `FUN-S-15` + `FUN-M-25` + `FUN-S-14`, y el minor del
> bloque sigue abierto hasta que entren.
