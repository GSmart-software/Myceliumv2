...
| `FUN-S-03` 🛠️ | `EXPLORER-EXTENSIONES` | Mostrar la extensión de los archivos no-markdown para poder identificarlos, **y listar los que Mycelium no indexa** (PDF, imágenes, código, texto): hasta ahora ni aparecían, así que el vault se veía más vacío de lo que está. **Implementado en desktop** el 2026-08-18 (sin confirmar). Se listan aparte de `store.notas` a propósito: meterlos ahí los metería en el autocompletado de `[[`, en la búsqueda y en el grafo. **Abrirlos sigue siendo `FUN-L-11`**; al hacer clic se avisa en vez de no hacer nada | ambas | C-M-12 |
