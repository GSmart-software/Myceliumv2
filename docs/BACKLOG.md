...
| `FUN-S-17` 🟢🌐 | `EXPLORER-GUIAS-INDENTACION` | **Líneas verticales de indentación** en el árbol del explorador, una por nivel de carpeta, que dibujan hasta dónde llega su contenido — el mismo recurso que usa VS Code. Con carpetas anidadas, la sangría sola no dice de qué carpeta cuelga una fila: hay que contar píxeles con la vista. La línea lo dice sin leer nada. **Sale del mismo sitio que ya lo resolvió** en el árbol de resultados de `FUN-M-20` (2026-09-05): la sangría tiene que darla el **anidamiento de contenedores** y no un `padding` calculado por nivel — con padding se ve igual, pero no hay ningún elemento que abarque la rama y no hay dónde colgar la línea. El explorador hoy usa `paddingLeft: depth * 14`, así que el trabajo real es reestructurar su render, no el CSS. **Implementado en desktop y confirmado en la app** el 2026-09-05, y **reflejado a web** el mismo día: `depth` desapareció de los cinco componentes por los que viajaba. Spec en [[busqueda-modos-y-arbol]] § 3 | ambas | — |
...
| `FUN-S-15` 🟢🌐 | `BASES-ORDENAR` | Ordenar las filas de un archivo tabla por **cualquiera de las columnas mostradas**, ascendente o descendente. **Implementado en desktop y confirmado en la app** el 2026-09-05, y **reflejado a web** el mismo día. El clic recorre ascendente → descendente → **sin orden** —volver al orden de la agregación tiene que estar en el ciclo— y <kbd>Shift</kbd>+clic **suma** la columna en vez de reemplazar, porque el formato admite varios criterios y un clic que siempre los borra dejaría un `sort` de dos columnas imposible de rehacer. Se guarda en el `sort` del archivo, que es donde el formato de Obsidian tiene sitio para esto. Con ello, un cambio deliberado: las celdas **sin valor van al final en las dos direcciones**. Spec en [[bases-tabla]] | ambas | — |
...
> repara los enlaces entrantes solo. Se conserva el aviso, tachado, porque explica **por qué
> se hizo en este orden** — no era una preferencia.
>
> Y no quedó «más simple», como decía esta nota hasta el 2026-09-05: al implementar
> `FUN-M-24` salió `DEF-084` —el renombrado reescribía los enlaces con un nombre que nunca
> existió en disco—, que es exactamente el riesgo del bloqueo por otra puerta. El orden
> tampoco sobró: sin `FUN-M-08` hecho y con el renombrado en el título, ese defecto habría
> sido cotidiano en vez de aparecer probando.
...
- **Qué es**: draw.io como tipo de archivo del vault —crear, abrir, editar y guardar un
  diagrama sin salir de Mycelium—, con el mismo trato que ya tiene un `.excalidraw`.
- **De dónde sale**: del usuario, en la [[Bandeja de entrada]] el 2026-09-22: «integrar la
  herramienta de draw.io en mycelium, así como se implementó excalidraw».
- **Por qué no duplica a Excalidraw**: Excalidraw manipula **trazos** y draw.io manipula
  **figuras conectadas**. Un diagrama de arquitectura que hay que retocar seis meses después
  es, en uno, mover trazos a mano; en el otro, mover una caja y que las flechas la sigan. Es
  el mismo argumento que separó a [[canvas]] de Excalidraw.
- **Por qué es L y no M**: el editor no se instala como componente. draw.io se integra como
  **aplicación web en modo embebido** (`iframe` con `?embed=1&proto=json`, diálogo por
...
> | **Añadida sobre la marcha** — `FUN-S-17` ✅ | Las guías de indentación del explorador. Se sumó el 2026-09-05, mientras se probaba el árbol de resultados de `FUN-M-20`: al verlas ahí, su ausencia en el explorador —que es donde más carpetas anidadas hay— pasó a ser lo raro. Va **después** de `FUN-M-20` y no junto a ella, porque no comparten archivo; lo que comparten es la técnica, y esa ya quedó resuelta y escrita |
>
...
`FUN-S-16` —buscador en el desplegable de campos— se sumó el mismo día y por el mismo
motivo: es del constructor de filtros, y hacerlo aparte significaría abrir ese componente
una vez más. Las tres cosas del constructor —negar, no borrar lo incompleto, y encontrar el
campo— salen mejor juntas que en tres pasadas.

> [!success] Bloque cerrado el 2026-09-05
> Las cinco están confirmadas en la app y reflejadas a web, en dos pasadas de un día:
> primero el **constructor de filtros** (`FUN-M-27` + `FUN-S-16` + `DEF-080`, en un
> componente nuevo, `components/bases/FiltrosBuilder.tsx`) y después la **cabecera de la
> tabla** (`FUN-S-15` + `FUN-S-14` + `FUN-M-25`).
>
> Agruparlas fue lo correcto por el motivo que decía el bloque, y las dos mitades lo
> confirmaron por separado: el árbol de `FUN-M-27` reescribió el constructor entero —el
> buscador y la marca de incompleta habrían tenido que rehacerse—, y las tres de la cabecera
> viven literalmente en el mismo `<th>`.
>
> `FUN-M-25` arrastró además una decisión que no estaba en el bloque: para no meter un
> import solo-desktop en `BaseView.tsx` hubo que portar las preferencias del vault a web
> (`FUN-M-29`), lo que de paso desatascó `FUN-M-28` y `FUN-M-21`.
